...
@main def checkParsers(): Unit =
  ...
  println("parserNTC.parseAll(\"XYZ\".toList).: " + parserNTC.parseAll("XYZ".toList)) // true
  println("parserNTC.parseAll(\"XYYZ\".toList): " + parserNTC.parseAll("XYYZ".toList)) // false
  println("parserNTC.parseAll(\"\".toList)....: " + parserNTC.parseAll("".toList)) // true

  // note we do not need a class name here, we use the structural type
  // Bottom-up decorations: NonEmpty[Char] -> NotTwoConsecutive[Char] -> BasicParser -> Parser[Char]
